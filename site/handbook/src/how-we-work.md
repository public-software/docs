...
| Fact | Value |
|---|---|
| Source of truth | `catalog/catalog.toml` in the [`catalog` repository](https://github.com/public-software/catalog), released as tag `v1.0.0`; its schema is [`catalog.schema.json`](https://publicsoftware.dev/catalog.schema.json) |
| Rings | {{RING_COUNTS}}; dependencies point inward only |
| Layers | {{LAYER_RANGE}} |
| Waves | {{WAVE_RANGE}}, the `wave` field of each repository's catalog entry |
| Cross-repository changes | an RFC in [`rfcs`](https://github.com/public-software/rfcs), by [RFC-0000](https://github.com/public-software/rfcs/blob/main/text/0000-rfc-process.md); the final comment period is 10 calendar days |
| Interfaces | WIT packages `public:<name>@<version>` in [`interfaces`](https://github.com/public-software/interfaces), published as `ghcr.io/public-software/public/<name>:<version>` ([RFC-0002](https://github.com/public-software/rfcs/blob/main/text/0002-wit-package-versioning.md)) |
| Sign-off | `git commit -s` on every commit (Developer Certificate of Origin); `main` requires signed commits |
| Licences | code `Apache-2.0 OR MIT`; content and specifications `CC-BY-4.0` |
| Reviews on `main` | one approving code-owner review, two in the platform ring; resolved threads; linear history |
| Required checks on `main` | `suite / pub-check`, `suite / probe`, `suite / test`, `suite / deny`, `suite / semver`, `suite / mutants`, `suite / vet`, `suite / audit`, `suite / typos`, `suite / miri`, `suite / policy` |
| Releases | branches `release/**`; immutable tags; trains named by year and number, the first `2027.1` |
| Provenance | `PROVENANCE.md` in every repository: what was consulted, and what was deliberately not |
| Canonical home | the organization's own forge, built in `forge`; GitHub is a mirror |

- A component starts from a specification, never from another implementation.
- A dependency points inward only: spine, platform, system, domain. Standards depend on the spine alone.
- A wave is assigned per repository in the catalog. Within a wave, work proceeds ring by ring.
- A change to the catalog is a pull request with a `CHANGELOG.md` line. Tooling pins a catalog tag, never `main`.
- A change that touches an interface or more than one repository goes through an RFC first.
- Every commit carries a sign-off and a signature. `main` takes changes by pull request only.
- A required check that has nothing to do reports success, so an empty repository is green.
- The `suite` repository pins every crate in one lockfile and builds the whole suite nightly.
...
`main` takes changes by pull request, with one approving review, code-owner review, resolved threads, signed commits and a linear history. Platform-ring repositories require two reviews. Release branches are `release/**`; release tags are immutable. The `suite` repository pins every crate in one lockfile and builds the whole suite nightly. Release trains are named by year and number; the first is `2027.1`.
